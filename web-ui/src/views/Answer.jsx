...
export default function Answer(){
    ...
    const getQuestionList = async () => {
        try{
            await axios.get(`/api/pageQuestionList/${pageType}/${pageNum}`,{
                headers:{
                ...
                if(data.code === 200){
                    if(data.data === null || data.data.length === 0)
                        ...
                        setQuestionList(questionList.concat(data.data))
                        setPageNum(pageNum+1)
                    }
                }else handleMsg("danger",data.message)
            })
        }catch(err){
            handleMsg("danger","获取问答列表失败")
        }
    }
    ...
    const getTop10 = async () => {
        await axios.get("/api/mostQuestionTop10",{
            headers:{
                token:store.getState().token
            }
        }).then(res=>res.data).then(data=>{
        ...
        }).catch(()=>handleMsg("danger","获取勤学好问榜失败"))
    }
        ...
        <>
            ...
            <Row className='bg-white rounded p-5'>
                <Col lg={{span:7}} md={{span:6}} xl={{span:8}}>
                    ...
                    {questionList && questionList.map((item,index)=><QuestionListItem item={item} key={index}/>)
                    }
                    {!questionList.length && <Row className='border-bottom mt-4'>
                        <Col md={{span:6,offset:3}}>
                            <Image src={empty} />
                        </Col>
                    </Row>}
                    {hasMore ? <p className='text-center text-primary'>
                        <span style={{cursor:"pointer"}} onClick={getQuestionList}>显示更多<i className="bi bi-caret-down"></i></span>
                            ...
                            {top10.map((item,index)=><ListGroup.Item as="li" key={index} className="d-flex">
                                <div className="d-flex justify-content-between" style={{width:"100%"}}>
                                    <div>
                                        ...
                                        <Badge className="mx-2">{item.questions}</Badge>
                                    </div>
